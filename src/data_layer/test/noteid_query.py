    ...
    async with async_db_session() as db:
        try:
            query = text(
                ...
                WHERE 
                    (npn.mohsNotes LIKE '%Filler (Filler)%')
                    OR (npn.biopsyNotes LIKE '%Filler (Filler)%')
                    OR (npn.procedure LIKE '%Filler (Filler)%')
...
def append_to_json(new_data, file_path="filler_id.json"):
    """
